...
function App() {
  ...
  useEffect(() => {
    const getAccountBalance = async () => {
      const accountsBalance = await web3Api.web3.eth.getBalance(`${account}`);
      setAccountBalance(web3Api.web3.utils.fromWei(accountsBalance, "ether"));
    };
  ...
  return (
    <>
      <div class="card text-center">
        <div class="card-header">The Funding Contract</div>
        <div class="card-body">
          <h5 class="card-title">Collected Funds: {balance} ETH </h5>
          <p class="card-text">
        ...
        <div class="card-footer text-muted">@Zaheer Khan</div>
      </div>
    </>
  );
}
